...
typedef struct
{
    lngrd_List *expressions;
    size_t index;
    lngrd_UInt ownership; /*(bitmask) 1,own-items 2,own-list*/
    lngrd_UInt phase;
...
LNGRD_API void lngrd_progress_executer(lngrd_Executer *executer, lngrd_Parser *parser)
{
    ...
    push_stash_item(flows, create_flow(expressions, 0, 0x3, 0, 0, 0));
        ...
        for (; flow->index < expressions->length; flow->index++, flow->phase = 0)
        {
            ...
            else if (expression->type == LNGRD_EXPRESSION_TYPE_INVOKE)
            {
                ...
                if (flow->phase < form->arguments->length)
                {
                    ...
                    push_stash_item(flows, create_flow(single, 0, 0x2, 0, flow->checkpoint, flow->capacity));
                    flow->phase += 1;
                    sustain = 1;
                ...
                else if (flow->phase == form->arguments->length)
                {
                    ...
                    push_stash_item(flows, create_flow(expressions, 0, 0x0, 0, checkpoint, flow->phase));
                    flow->phase += 1;
                    sustain = 1;
            ...
            else if (expression->type == LNGRD_EXPRESSION_TYPE_ARGUMENT)
            {
                ...
                else if (flow->phase == 1)
                {
                    ...
                    if (index->value < 1 || (lngrd_UInt) index->value >= flow->capacity)
                    {
                        set_executer_error("absent argument", executer);
                    ...
                    set_executor_result(executer->arguments->items[flow->checkpoint - flow->capacity + index->value], executer);
                    flow->phase = 2;
                    sustain = 1;
            ...
            else if (expression->type == LNGRD_EXPRESSION_TYPE_GROUP)
            {
                ...
                push_stash_item(flows, create_flow(form->expressions, 0, 0x0, 0, flow->checkpoint, flow->capacity));
                flow->index += 1;
                flow->phase = 0;
                sustain = 1;
            ...
            else if (expression->type == LNGRD_EXPRESSION_TYPE_NATIVE)
            {
                ...
                form->work(executer, arguments, flow->capacity);
    ...
    if (flows->length > 0 || arguments->length > 0 || locals->length > 1)
    {
...
static void burn_flow(lngrd_Flow *flow, lngrd_List *pyre)
{
    if (flow->expressions)
    {
        if (flow->ownership & 0x1)
        {
            while (flow->expressions->length > 0)
            {
                push_list_item(pyre, pop_list_item(flow->expressions));
            }
        }

        if (flow->ownership & 0x2)
        {
